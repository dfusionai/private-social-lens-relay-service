...
export class WalrusService {
  ...
  constructor(
    ...
    this.walrusClient = new WalrusClient({
      ...
      uploadRelay: {
        ...
        sendTip: {
          max: 5_000_000, // in MIST (1 SUI equals 1 billion MIST)
        },
      },
    });
  }

  // async uploadFileViaRelay(file, uploadDto: WalrusUploadRequestDto) {
  //   try {
  ...
  // async uploadFileViaRelayHybrid(
  //   file: Express.Multer.File,
  //   uploadDto: WalrusUploadRequestDto,
  // ) {
  //   try {
  //     this.logger.log('Starting hybrid file upload process');
  //     const signer = this.suiWalletService.getKeypair();
  //     if (!signer) {
  //       throw new Error('No Sui signer/keypair is initialized');
  //     }
  ...
  //     // Encode and store metadata for relay upload
  //     const encodedMetadata = await flow.encode();
  //     this.logger.log('File encoded successfully');

  //     // Extract blobId and nonce for relay upload
  //     const blobId = encodedMetadata.metadata.blobId;
  //     const nonce = encodedMetadata.metadata.nonce;
  ...
  //       epochs: Number(uploadDto.epochs),
  //       deletable: true,
  //       owner: signer.toSuiAddress(), // Service owns the blob
  //     });
  ...
  //     // Step 3: Upload file data to the relay instead of direct storage nodes
  //     this.logger.log('Uploading file data to relay');
  //     await this.uploadToRelay({
  //       file,
  //       blobId,
  ...
  //   } catch (error) {
  //     this.logger.error(
  //       'Failed to upload file to Walrus via hybrid approach.',
  //       error,
  //     );
  //     throw new Error(
  //       'Failed to upload file to Walrus via hybrid approach. Please try again.',
  //     );
  //   }
  // }

  // private async uploadToRelay({
  //   file,
  //   blobId,
  //   txDigest,
  //   nonce,
  ...
  //   nonce: Uint8Array;
  // }) {
  //   const relayUrl = `${this.configService.get<string>('sui.walrus_relay_url')}/v1/blob-upload-relay`;

  //   // Build the URL with query parameters
  //   const url = new URL(relayUrl);
  //   url.searchParams.append('blob_id', blobId);
  //   url.searchParams.append('tx_id', txDigest);
  //   url.searchParams.append('nonce', Buffer.from(nonce).toString('base64'));

  //   // Upload the file
  //   const response = await fetch(url.toString(), {
  //     method: 'POST',
  //     body: file.buffer, // Use the buffer from the Multer file
  //     headers: {
  ...
  //       errorText,
  //     );
  //     throw new Error(`Relay upload failed: ${response.statusText}`);
  //   }
